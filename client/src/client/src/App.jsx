        ...
        <div className="card">
          ...
          <div style={{ marginTop: '20px' }}>
            ...
            <form onSubmit={handleLogin}>
              <input type="text" placeholder="Enter Wallet Address" value={wallet} onChange={(e) => setWallet(e.target.value)} required />
            ...
            <form onSubmit={handleRegister}>
              ...
              <button type="submit" className="btn-gold" style={{ width: '100%' }} disabled={loading}>{loading ? 'Generating ID...' : 'Register & Get KS1 ID'}</button>
            </form>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="container">
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap:'wrap', gap:'1rem' }}>
        ...
        <button onClick={() => { setUserData(null); setView('register'); setWallet(''); }} className="btn-gold" style={{ padding: '8px 16px', fontSize: '0.8rem' }}>Logout</button>
      </header>
